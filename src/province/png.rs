    ...
    pub fn to_rgba_image(&self, provinces: &[Province]) -> Vec<u8> {
        ...
        for province in provinces {
                ...
                )
            {
                ...
                // Fallback‑цвет для аномальных HEX‑значений:
                // - Если цвет не распознаётся (не 6 цифр/букв)
                // - ИЛИ если значения выходят за диапазон (потому что from_str_radix возвращает Err)
                // Используем тёмно‑серый как нейтральный fallback
                color_map.insert(province.id, [0, 0, 0, 255]);
            }
        }
